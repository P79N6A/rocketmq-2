...
public class BloomFilter {
    ...
    /**
     * Create bloom filter by error rate and mapping num.创建布鲁姆过滤器的错误率和映射数字。
     *
     * @param f error rate
...
//
     ...
     * </p>
     * 计算str的位位置。
     参见亚当•基尔希(Adam Kirsch)和迈克尔•米岑马赫(Michael Mitzenmacher)的《减少哈希，同样的性能:构建更好的布卢姆过滤器》(Less hash, Same Performance: Building a Better Bloom Filter)。
     */
    ...
    /**
     * Calculate bit positions of {@code str} to construct {@code BloomFilterData} 计算str的位位置来构造BloomFilterData
     */
    ...
    /**
     * Calculate bit positions of {@code str}, then set the related {@code bits} positions to 1.计算str的位位置，然后将相关位位置设为1。
     */
    ...
    /**
     * Calculate bit positions of {@code str}, then check all the related {@code bits} positions is 1.计算str的位位置，然后检查所有相关位位置是否为1。
     *
...
//
    ...
    /**
     * Check whether one of {@code bitPositions} has been occupied.检查一个位是否被占用。
     *
