 ...
 * </p>
 * 对两个对象进行比较的过滤器
 这个类来自ActiveMQ org.apache.activemq.filter。ComparisonExpression,但是:1。删除LIKE表达式及相关方法;2。提取具有int返回值的新方法__compare;3所示。在表达式之间创建时，检查左值是否小于或等于右值;4所示。对于字符串类型值(不能转换为数字)，只支持相等或不等的比较。
 */
//
...
//
    ...
    /**
     * Only Numeric expressions can be used in >, >=, < or <= expressions.s 在>、>=、<或<= express .s中只能使用数值表达式
     */
//
     ...
     * not be NULL TRUE or FALSE litterals.验证表达式可以在==或<>表达式中使用。不能是无效的真或假的乱丢垃圾。
     */
//
