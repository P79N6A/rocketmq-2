...
//    是否自动订阅消费组
     ...
     * value is 1.发送消息线程池的线程数，因为自4.0开始，自旋锁将默认使用。x，默认值是1。
     */
//    发送消息线程池数
...
//    开始接受发送请求的时间
    ...
    // Expect num of consumers will use filter.期望用filter的消费者数量
    private int expectConsumerNumUseFilter = 32;

    // Error rate of bloom filter, 1~100.
    private int maxErrorRateOfBloomFilter = 20;

    //how long to clean filter data after dead.Default: 24h 24小时清楚一次失效的过滤数据
    private long filterDataCleanTimeSpan = 24 * 3600 * 1000;

    // whether do filter when retry.filter是否支持重试
...
//    是否开启属性过滤
     ...
     * between 10, 000 and 60, 000 milliseconds.此可配置项定义了代理注册到命名服务器的主题间隔。允许的值在10,000到60,000毫秒之间。
     */
    private int registerNameServerPeriod = 1000 * 30;

    /**
     * The minimum time of the transactional message  to be checked firstly, one message only exceed this time interval
     * that can be checked.首先检查事务性消息的最小时间，一条消息仅超过这个可以检查的时间间隔。
     */
    @ImportantField
    private long transactionTimeOut = 3 * 1000;

    /**
     * The maximum number of times the message was checked, if exceed this value, this message will be discarded.检查消息的最大次数，如果超过此值，将丢弃此消息。
     */
    @ImportantField
    private int transactionCheckMax = 5;

    /**
     * Transaction message check interval.事务消息检查间隔。
     */
    @ImportantField
