 ...
 * 扩展消费队列，存储一些不重要的东西，如消息存储时间，过滤位图等。

 1。这个类只被ConsumeQueue使用
 2。这周很可靠。
 3所示。注意，返回的地址总是小于0。
 4所示。请将此档案缩小。
 */
...
public class ConsumeQueueExt {
    ...
    /**
     * Addr can not exceed this value.For compatible.Addr不能超过此值。兼容的。
     */
...
//
     ...
     * </p>
     * 转换地址(通过修饰修饰)以在映射文件中偏移。
     如果地址小于0，返回地址- Long.MIN_VALUE;否则，只返回地址
     */
//
     ...
     * </p>
     * 修饰映射文件的偏移量，以便与标记代码(最初保存在cq中)区分开来。
     如果偏移量大于或等于0，则返回offset + Long.MIN_VALUE;否则，返回偏移量
     *
...
//
    ...
    /**
     * Check whether the step size in mapped file queue is correct.检查映射文件队列中的步长是否正确。
     */
...
//        映射文件队列设置offset commit的位置
        this.mappedFileQueue.setCommittedWhere(processOffset);
//        映射文件队列按处理offset删除脏数据文件=》
...
//        删除期望的文件=》
    ...
    /**
     * Delete files after {@code maxAddress}, and reset wrote/commit/flush position to last file.删除maxAddress后的文件，并将写入/提交/刷新位置重置为最后一个文件。
     *
...
//
    ...
    /**
     * Max address(value is less than 0).最大地址(值小于0)。
     * <p/>
     * <p>
    ...
    public static class CqExtUnit {
        ...
        /**
         * the time to store into commit log of message 存储到消息提交日志中的时间
         */
        ...
        /**
         * build unit from buffer from current position.从当前位置从缓冲区构建单元。
         */
//
         ...
         * 将单元数据转换为字节数组。
         ...
         1。@{code container}可以为空，如果为空则创建。
         2。如果@{code container}的容量小于单位大小，也会创建。
         3所示。请确保单元的大小不大于MAX_EXT_UNIT_SIZE
         */
