...
public class MessageStoreConfig {
    //The root directory in which the log data is kept 消息存储根目录
    @ImportantField
    private String storePathRootDir = System.getProperty("user.home") + File.separator + "store";

    //The directory in which the commitlog is kept commitLog存储目录
    @ImportantField
    private String storePathCommitLog = System.getProperty("user.home") + File.separator + "store"
        + File.separator + "commitlog";

    // CommitLog file size,default is 1G commitLog文件大小，默认1G
    private int mapedFileSizeCommitLog = 1024 * 1024 * 1024;
    // ConsumeQueue file size,default is 30W 消息队列大小 默认30w
    private int mapedFileSizeConsumeQueue = 300000 * ConsumeQueue.CQ_STORE_UNIT_SIZE;
    // enable consume queue ext
    private boolean enableConsumeQueueExt = false;
    // ConsumeQueue extend file size, 48M 消费队列扩展文件大小 48M
    ...
    // flush data to disk commitLog刷新到磁盘的时间
    @ImportantField
    private int flushIntervalCommitLog = 500;

    // Only used if TransientStorePool enabled
    // flush data to FileChannel 刷新数据到fileChannel的时间
    @ImportantField
    private int commitIntervalCommitLog = 200;

    /**
     * introduced since 4.0.x. Determine whether to use mutex reentrantLock when putting message.<br/>
     * By default it is set to false indicating using spin lock when putting message. 介绍了自4.0.x。确定在放置消息时是否使用互斥锁。在默认情况下，它被设置为假指示使用旋转锁定时，放置消息。
     */
//    存储消息是否使用互斥锁，默认使用自旋锁
    private boolean useReentrantLockWhenPutMessage = false;

    // Whether schedule flush,default is real-time 是否定时刷新，默认实时
    @ImportantField
    private boolean flushCommitLogTimed = false;
    // ConsumeQueue flush interval 消息队列刷新频次
    private int flushIntervalConsumeQueue = 1000;
    // Resource reclaim interval 资源回收频次
    private int cleanResourceInterval = 10000;
    // CommitLog removal interval commitLog删除频次
    private int deleteCommitLogFilesInterval = 100;
    // ConsumeQueue removal interval 消息队列删除频次
    private int deleteConsumeQueueFilesInterval = 100;
//    强行删除映射文件队列频次
    private int destroyMapedFileIntervalForcibly = 1000 * 120;
//    重新删除无用的文件频次
    private int redeleteHangedFileInterval = 1000 * 120;
    // When to delete,default is at 4 am 默认上午4点删除
    @ImportantField
    private String deleteWhen = "04";
//    磁盘默认最大空间使用率是75%
    private int diskMaxUsedSpaceRatio = 75;
    // The number of hours to keep a log file before deleting it (in hours) 日志文件保留72小时
    @ImportantField
    private int fileReservedTime = 72;
    // Flow control for ConsumeQueue
    private int putMsgIndexHightWater = 600000;
    // The maximum size of a single log file,default is 512K 单个日志最大512k
    ...
    //是否检查所消耗记录的CRC32。
//这确保不会发生消息的联机或磁盘损坏。
//这个检查增加了一些开销，所以在寻求极端性能的情况下可能会禁用它。
    private boolean checkCRCOnRecover = true;
    // How many pages are to be flushed when flush CommitLog 最少刷新commitLog的页数
    private int flushCommitLogLeastPages = 4;
    // How many pages are to be committed when commit data to file 最少自动提交commitLog的页数
    private int commitCommitLogLeastPages = 4;
    // Flush page size when the disk in warming state 当磁盘处于升温状态刷新页面大小
    private int flushLeastPagesWhenWarmMapedFile = 1024 / 4 * 16;
    // How many pages are to be flushed when flush ConsumeQueue 在刷新ConsumeQueue时要刷新多少页
...
//
     ...
     * ASYNC_FLUSH 只有当transientStorePoolEnable为true且FlushDiskType为ASYNC_FLUSH时，才启用瞬态commitLog存储轮询
     *
