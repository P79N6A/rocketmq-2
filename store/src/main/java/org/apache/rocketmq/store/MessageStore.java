...
/**
 * This class defines contracting interfaces to implement, allowing third-party vendor to use customized message store.该类定义要实现的契约接口，允许第三方供应商使用定制的消息存储。
 */
//
public interface MessageStore {
    ...
    /**
     * Destroy this message store. Generally, all persistent files should be removed after invocation.销毁此消息存储。通常，所有持久文件都应该在调用之后删除。
     */
    void destroy();
    ...
    /**
     * Get the offset of the message in the commit log, which is also known as physical offset.获取提交日志中消息的偏移量，也称为物理偏移量。
     *
    ...
    /**
     * Look up the physical offset of the message whose store timestamp is as specified.查找指定存储时间戳的消息的物理偏移量。
     *
    ...
    /**
     * Look up the message by given commit log offset.根据给定的提交日志偏移量查找消息。
     *
    ...
    /**
     * Get one message from the specified commit log offset.从指定的commit日志偏移量获取一条消息。
     *
    ...
    /**
     * Get the running information of this store.获取该商店的运行信息。
     *
    ...
    /**
     * Message store runtime information, which should generally contains various statistical information.消息存储运行时信息，通常应该包含各种统计信息。
     *
    ...
    /**
     * Get the store time of the earliest message in the given queue.获取给定队列中最早消息的存储时间。
     *
    ...
    /**
     * Get the store time of the earliest message in this store.获取该存储中最早消息的存储时间。
     *
    ...
    /**
     * Get the store time of the message specified.获取指定消息的存储时间。
     *
    ...
    /**
     * Get the total number of the messages in the specified queue.获取指定队列中的消息总数。
     *
     * @param topic Topic
     * @param queueId Queue ID.
     * @return total number.
    ...
    /**
     * Get the raw commit log data starting from the given offset, which should used for replication purpose.从给定的偏移量开始获取原始提交日志数据，该偏移量应该用于复制目的。
     *
    ...
    /**
     * Execute file deletion manually.手动执行文件删除。
     */
    ...
    /**
     * Return how much the slave falls behind.返回奴隶落后多少。
     *
    ...
    /**
     * Return the current timestamp of the store.返回商店的当前时间戳。
     *
    ...
    /**
     * Check if the given message has been swapped out of the memory.检查给定的消息是否已从内存中交换。
     *
     * @param topic topic.
     * @param queueId queue ID.
    ...
    /**
     * Get number of the bytes that have been stored in commit log and not yet dispatched to consume queue.获取已存储在commit日志中但尚未分配到使用队列的字节数。
     *
    ...
    /**
     * Flush the message store to persist all data.刷新消息存储以保存所有数据。
     *
    ...
    /**
     * Check if the operation system page cache is busy or not.检查操作系统页面缓存是否繁忙。
     *
    ...
    /**
     * Get lock time in milliseconds of the store by far.获取锁定时间(以毫秒为单位)。
     *
