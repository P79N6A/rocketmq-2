...
public interface MessageFilter {
     ...
     * and stored in consume queue ext.通过标记代码或过滤位图进行匹配，当消息接收并存储在使用队列ext中时，将计算此映射。
     *
     ...
     * 根据存储在commit日志中的消息内容进行匹配。msgBuffer和属性不都是空的。如果在store中调用，属性为null;如果在PullRequestHoldService中调用，msgBuffer为null。
     *
