...
/**
 * Store all metadata downtime for recovery, data protection reliability 存储所有元数据停机恢复，数据保护可靠性
 */
//
...
//            查询消息所在映射buffer
    ...
    /**
     * When the normal exit, data recovery, all memory data have been flush 当正常退出、数据恢复时，所有内存数据都已刷新
     */
//
...
//
    ...
    /**
     * check the message and returns the message size 检查消息并返回消息大小
     *
...
//        得到映射文件队列中的最后一个文件=》
        ...
        //fine-grained lock instead of the coarse-grained 得到批量消息编码器
...
//        消息集群刷新=》
    ...
    /**
     * According to receive certain message or offset storage time if an error occurs, it returns -1 如果发生错误，根据接收到的消息或偏移存储时间，返回-1
     */
//
