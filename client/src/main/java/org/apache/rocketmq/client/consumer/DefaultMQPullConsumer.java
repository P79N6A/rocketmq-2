...
public class DefaultMQPullConsumer extends ClientConfig implements MQPullConsumer {
     ...
     * guarantee Globally unique 对同一组做同样的事情，应用程序必须被设置，并保证全局唯一
     */
    private String consumerGroup;
    /**
     * Long polling mode, the Consumer connection max suspend time, it is not
     * recommended to modify 长轮询模式，用户连接最大暂停时间，不建议修改
     */
    private long brokerSuspendMaxTimeMillis = 1000 * 20;
    /**
     * Long polling mode, the Consumer connection timeout(must greater than
     * brokerSuspendMaxTimeMillis), it is not recommended to modify 长轮询模式，使用者连接超时(必须大于brokerSuspendMaxTimeMillis)，不建议修改
     */
