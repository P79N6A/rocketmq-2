 ...
 * 一种基于机房近端优先级的分配策略代理。可以指定实际的分配策略。如果机房中有任何使用者，那么部署在同一台机器上的代理的消息队列应该只分配给那些使用者。否则，这些消息队列可以在所有消费者之间共享，因为没有活着的消费者来垄断它们
 */
//
...
//
    ...
    public List<MessageQueue> allocate(String consumerGroup, String currentCID, List<MessageQueue> mqAll,
        ...
        //group mq by machine room 把消息队列部署所在机房进行分组
...
//                    如果没找到消息队列所在机房
        ...
        //group consumer by machine room 把消费者按消息队列所在部署机房进行分组
...
//                    如果没找到消费者所在的机房
        ...
        //1.allocate the mq that deploy in the same machine room with the current consumer 分配与当前使用者部署在同一机房中的mq
//        获取当前client所在的机房
...
//        获取当前机房的消费者
        ...
        //2.allocate the rest mq to each machine room if there are no consumer alive in that machine room 如果该机房中没有存活的使用者，则将其余mq分配给每个机房
        for (String machineRoom : mr2Mq.keySet()) {
            if (!mr2c.containsKey(machineRoom)) { // no alive consumer in the corresponding machine room, so all consumers share these queues 在相应的机房中没有活动的消费者，因此所有消费者共享这些队列
     ...
     * 解析器对象，以确定在哪个机房部署消息队列或客户机。附近的allocatemachineroomom将使用结果将消息队列和客户机按机房分组。从实现的方法返回的结果不能为空。
     */
//
